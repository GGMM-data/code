...
class Agent(BaseModel):
	...
	def train(self):
		...
		for self.step in tqdm(range(start_step, self.max_step), ncols=70, initial=start_step):
			...
			# sample
			# 1. predict
			action = self.predict(self.history.get())  # behaviour policy, epsilon-greedy
			# 2. act
	...
	def build_dqn(self):
		...
		with tf.variable_scope('optimizer'):
			# 目标q值，taget_q = r + γ q(s_t_1, a_t_1)
