...
class inflow_outflow():
    ...
    def create_inflow_outflow(self):     
        for file in self.files:
            ...
            _ = self.process_long_lati_one_trajectory_didi(trajectory, inflow_outflow_raw_data_pathname)
    ...
    def create_time_series(self):
        ...
        for i, data in enumerate(time_series):
            if time_series[i] == last_time:
                series.append(str(time_series[i]) + str(count).zfill(4))
            else:
                count = 1
...
class two_flow(flow):
    ...
    def count_in_out_flows(self,in_out_flow_raw_data):
        ...
        for i, item in enumerate(in_out_flow_raw_data):
            ...
            if area[0] > 31 or area[1] > 31:
                continue
            if time_slot > self.nb_timeslot - 1:
                continue
            if inflow_change == 1:
...
def process(pattern, parent_dir, begin_long, end_long, begin_lati, end_lati, begin_time, end_time, time_interval, m,n, nb_flows):
    # h5文件的存储路径
    h5_file_name = "./info_didi/TaxiDD_xian_" + str(begin_long) + "_" + str(end_long) + "_" + str(begin_lati) + "_" + str(end_lati) + "_" + str(time_interval) + "_" + str(m) + "_" + str(n) + "_" + str(nb_flows) + "_count.h5"

    # 对应的h5文件的信息
    with open(h5_file_name + "info.txt", "w") as f:
...
def main():
    begin_time = 1475251200
    end_time = 1480521599
    time_interval = 1800
    m = n = 32

    # [108.92309, 34.279936],[109.008833, 34.278608],
    # [108.921859, 34.204946], [109.009348, 34.207309].
    begin_long = 108.93
    end_long = 109.01
    mid_long = 108.97
    begin_lati = 34.20
    ...
    nb_flows = 2
    long = [begin_long, mid_long, end_long]
    lati = [begin_lati, mid_lati, end_lati]

    pattern = "\'(.*?)'"
    parent_dir = r"E:\\data\\didi\\xian\\single_driver_data\\"

    for i in range(2):
        for j in range(2):
            process(pattern, parent_dir, long[i], long[i+1], lati[j], lati[j+1], begin_time, end_time, time_interval, m,n, nb_flows)
